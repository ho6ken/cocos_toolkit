...
export class NetNode {
    ...
    public startConn(opt: NetConnOpt): boolean {
        if (this._state != NetState.Closed) {
            console.warn(`start connect failed, it's wrong state.`, opt.addr);
            return false;
        }

        console.log(`start connect`, opt.addr);
    ...
    private initSocket(): void {
        ...
        // 收到訊息
        this._socket.onMsg = buf => {
            // 非法數據
            if (!this._helper.isLegal(buf)) {
                console.warn(`rcv illegal msg.`, buf);
                return;
            }
        ...
        // 連線成功
        this._socket.onConnect = event => {
            console.log(`connect succeed.`, this._connOpt.addr);
        ...
        // 連線異常
        this._socket.onErr = event => {
            console.error(`connect err.`, this._connOpt.addr, event);
        };
        ...
        this._socket.onClose = event => {
            ...
            this._reconnTimer = window.setTimeout(() => {
                ...
                // 無剩餘重連次數
                if (this._chances <= 0) {
                    console.warn(`reconnect failed, run out of chances.`, this._connOpt.addr);
                    this.clearTimers();
                    return;
                }

                console.warn(`start reconnect.`, this._connOpt.addr);
    ...
    public closeConn(code?: number, reason?: string): void {
        console.warn(`close connect.`, code, reason);
        ...
        switch (this._state) {
            ...
            case NetState.Connected: {
                if (this._socket.sendMsg(buf)) {
                    console.log(`send msg succeed.`, cmd, this._connOpt.addr, buf);
                    return true;
                } 
                else {
                    console.warn(`send msg failed.`, cmd, this._connOpt.addr, buf);
                    return false;
                }
            }
            ...
            case NetState.Resending: {
                ...
                console.warn(`send msg later.`, cmd, this._connOpt.addr, buf);
                return true;
            }
    ...
    public sendReq(cmd: NetCmd, buf: NetBuf, resp: NetHandler): void {
        ...
        if (this._state != NetState.Connected) {
            console.warn(`send req later.`, cmd, this._connOpt.addr, buf);
            return;
        }

        this._socket.sendMsg(buf);
        console.log(`send req succeed.`, cmd, this._connOpt.addr, buf);
    }
    ...
    public sendUnique(cmd: NetCmd, buf: NetBuf, resp: NetHandler): boolean {
        ...
        for (const req of this._requests) {
            if (req.cmd == cmd) {
                console.warn(`send unique failed, cmd repeat.`, cmd, this._connOpt.addr, buf);
                return false;
            }
        }

        this.sendReq(cmd, buf, resp);
        console.log(`send unique succeed`, cmd, this._connOpt.addr, buf);
